...
# ── Temperature Tuning Configuration ────────────────
    # Testing different temperature scales for the 3-layer no BN setup
    temperatures = [0.01, 0.05, 0.1, 0.5, 1.0, 2.0]
    
    for temp in temperatures:
        print(f"\n>>> Tuning Temperature: {temp} (Layers: 3, BN: False)")
        ...
        model = MNISTModel(
            ...
            temperature=temp,      # Using the temperature from the loop
            hidden_layers=3,       # Fixed at 3 layers
            use_batch_norm=False   # BN disabled
        )
        ...
        # Evaluation and logging to results list
    ...
    df.to_csv("tune_temperature_results.csv", index=False)
    print("\nTesting complete. Results saved to tune_temperature_results.csv")
